...
use fixed_dsp::transform::{RfftI16, RfftI32};
...
unsafe extern "C" {
    ...
    static realCoefAQ31: i32;
    static realCoefBQ31: i32;
...
fn sample_real_i32_buffer(fft_len: usize) -> Vec<i32> {
    (0..fft_len)
        .map(|index| {
            (index as i32)
...
fn cmsis_rfft_q31_forward(input: &[i32], output: &mut [i32], bit_reverse_flag: bool) {
    let fft_len_real = input.len();
    let fft_len = fft_len_real >> 1;
    let modifier = (8192 / fft_len_real) as u32;

    let mut scratch = input.to_vec();
    cmsis_cfft_q31(&mut scratch, fft_len, false, bit_reverse_flag);

    unsafe {
        arm_split_rfft_q31(
            scratch.as_mut_ptr(),
            fft_len as u32,
            &raw const realCoefAQ31,
            &raw const realCoefBQ31,
            output.as_mut_ptr(),
            modifier,
        );
    }
...
fn cmsis_rfft_q31_inverse(input: &[i32], output: &mut [i32], bit_reverse_flag: bool) {
    let fft_len_real = output.len();
    let fft_len = fft_len_real >> 1;
    let modifier = (8192 / fft_len_real) as u32;
    ...
    cmsis_cfft_q31(output, fft_len, true, bit_reverse_flag);

    unsafe {
        arm_shift_q31(output.as_ptr(), 1, output.as_mut_ptr(), fft_len_real as u32);
    }
}

unsafe extern "C" {
    fn arm_split_rfft_q31(
        pSrc: *mut i32,
        fftLen: u32,
        pATable: *const i32,
        pBTable: *const i32,
...
#[test]
fn rfft_q31_forward_difftest_against_cmsis() {
    for &fft_len_real in &[32usize, 64, 128, 256, 512, 1024, 2048, 4096, 8192] {
        for &bit_reverse_flag in &[false, true] {
            ...
            assert_eq!(
                rust_output, cmsis_output,
                "Q31 RFFT mismatch fft_len_real={}, bitrev={}",
                fft_len_real, bit_reverse_flag
            );
        }
    }
...
#[test]
fn rfft_q31_inverse_difftest_against_cmsis() {
    for &fft_len_real in &[32usize, 64, 128, 256, 512, 1024, 2048, 4096, 8192] {
        for &bit_reverse_flag in &[false, true] {
            let input_time = sample_real_i32_buffer(fft_len_real);
            let mut spectrum = vec![0i32; fft_len_real * 2];
            cmsis_rfft_q31_forward(&input_time, &mut spectrum, bit_reverse_flag);

            let mut rust_output = vec![0i32; fft_len_real];
            ...
            assert_eq!(
                rust_output, cmsis_output,
                "Q31 RIFFT mismatch fft_len_real={}, bitrev={}",
                fft_len_real, bit_reverse_flag
            );
        }
    }
