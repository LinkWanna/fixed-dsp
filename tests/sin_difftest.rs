...
#[test]
fn sin_q15_difftest_against_cmsis() {
    let inputs: Vec<i16> = (i16::MIN..=i16::MAX).step_by(17).collect();
    let stats = run_i16_difftest(&inputs, sin_i16, |x| unsafe { arm_sin_q15(x) });
    println!("q15 stats: {:?}", stats);

    assert_max_abs_diff(stats, 6, "sin_i16 vs arm_sin_q15");
}

#[test]
fn sin_q31_difftest_against_cmsis() {
    let inputs = sample_q31_inputs();
    let stats = run_i32_difftest(&inputs, sin_i32, |x| unsafe { arm_sin_q31(x) });
    println!("q31 stats: {:?}", stats);

    assert_max_abs_diff(stats, 50_000, "sin_i32 vs arm_sin_q31");
}
